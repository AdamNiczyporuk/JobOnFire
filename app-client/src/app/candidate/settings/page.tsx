...
export default function CandidateSettingsPage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await deleteAccount();
      // Upewnij się, że kontekst auth jest wyczyszczony
      setUser(null);
      // Dla pewności wywołaj logout (gdyby sesja pozostała)
      await logout().catch(() => {});
      ...
      <section className="rounded-lg border bg-white p-6 shadow-sm mb-8">
        ...
        {user && user.hasPassword === false ? (
          <div>
            ...
            <p className="text-sm">Aby zarządzać dostępem, skorzystaj z ustawień konta Google lub utwórz hasło, korzystając z mechanizmu przywracania/ustawienia hasła (jeśli będzie dostępny).</p>
          </div>
        ) : (
